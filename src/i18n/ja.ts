...
const lang = {
  ...
  mintPanel: {
    ...
    maxLength: "最大32バイト。",
    cc0Message:
      "ここで投稿した画像は、cc-Share Earnings（自由にレミックス可能、売上の分配が必要）として他の方に自由に再利用・加工していただくことが可能になりますので、そこに同意した上でミントして下さい。",
    drawMessage:
  ...
  draw: {
    ...
    message5:
      "リミックスやオーバーレイをした作品をミントする場合は有料({ mintPrice} ETH)で、そのお金はクリエーターたちに配布されます。",
    message6:
